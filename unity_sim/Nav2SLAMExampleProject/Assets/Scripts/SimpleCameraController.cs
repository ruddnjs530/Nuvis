...
namespace Unity.RenderStreaming
{
    ...
    public class SimpleCameraController : MonoBehaviour
    {
        ...
        Vector3 GetInputTranslationDirection()
        {
            ...
            if (m_keyboard.wKey.isPressed) direction += Vector3.forward;
            if (m_keyboard.sKey.isPressed) direction += Vector3.back;
            if (m_keyboard.aKey.isPressed) direction += Vector3.left;
            if (m_keyboard.dKey.isPressed) direction += Vector3.right;
            if (m_keyboard.qKey.isPressed) direction += Vector3.down;
            if (m_keyboard.eKey.isPressed) direction += Vector3.up;
        ...
        static bool IsMouseDragged(Mouse m, bool useLeftButton)
        {
            ...
            if (Screen.safeArea.Contains(m.position.ReadValue()))
            {
                if ((useLeftButton && m.leftButton.isPressed) ||
                    (!useLeftButton && m.rightButton.isPressed))
                {
                    return true;
                }
            }
