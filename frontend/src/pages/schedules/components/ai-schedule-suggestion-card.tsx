...
import { Cancel01Icon, GridViewIcon, MagicWand01Icon, Time02Icon } from '@hugeicons/core-free-icons';
...
export default function AiScheduleSuggestionCard({ suggestion, onClick, onDismiss, className }: AiScheduleSuggestionCardProps) {
  const deviceName = suggestion.actionModuleType === 'AIR_PURIFIER' ? '공기청정기' : '가습기';
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] font-bold uppercase tracking-widest text-brand/70">AI Schedule Insight</span>
              <span className="text-xs font-extrabold text-fg-strong">{suggestion.pattern}</span>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1 mt-0.5">
          <div className="flex items-center gap-1 text-fg-muted">
            <Icon icon={GridViewIcon} size="xs" color="currentColor" className="h-3 w-3" />
          ...
          <div className="flex items-center gap-1.5 text-xl font-black tracking-tight text-fg-strong leading-tight">
            ...
            <span>
              매일
              <span className="text-brand underline underline-offset-2 decoration-brand/30">{suggestion.time}</span>
              {' '}
              시작
            </span>
          </div>
        ...
        <div className="rounded-xl bg-brand/5 p-3 text-xs leading-relaxed text-fg-default border border-brand/5 relative mt-1">
          <p className="line-clamp-2 font-medium opacity-90">{suggestion.reason}</p>
        </div>
      </button>
