...
- [x] 추천 서버 다중 방(`room_id`) 방별 분석 구조 반영
  - 기존 요청 형식 유지, 응답은 `room_id`별 결과 맵으로 확장
- [x] 다중 방 mock payload 생성기 정리 및 재생성
  - `server_ai/recommendation/data/mock_payload.json` 기준 `1~9 room_id`, 총 `1521`건 생성 완료
...
### 2026-03-27
- [x] **[Feature]** 추천 서버 다중 방(`room_id`) 분석 구조 고도화 완료
  - `room_id`별 `DataFrame` 분리 후 방마다 이벤트/스케줄 추천을 수행하도록 정리
  - 기존 요청 포맷은 유지하고, 응답만 `room_id`별 결과 맵으로 확장
- [x] **[Feature]** 다중 방 시뮬레이션용 mock payload 생성기 재작성 완료
  - 9개 방 기준 생활 패턴/환경 편향이 다른 가짜 시계열 데이터 생성
  - 기본 출력 경로를 `server_ai/recommendation/data/mock_payload.json` 하나로 정리
  - 14일, 2시간 간격, 총 `1521`건 생성 검증 완료
...
- [x] **[Fix]** 백엔드 업데이트에 따른 추천 서버 데이터 필드명 호환성(Alias) 조치 완료
  - Pydantic `Field(alias="...")` 적용으로 `userId`, `data`, `roomId` 키 불일치 해결
  - 백엔드 팀 연동 가이드 및 요청 메시지 작성
